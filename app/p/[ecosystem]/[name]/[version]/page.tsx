...
const PackageDetails = ({ title, children }: { title: string, children: React.ReactNode }) => {
    return (
        <div className="flex items-center justify-center gap-1">
            <div className="flex items-center justify-center gap-1">
                <div className="size-1 md:size-2 bg-[#069c85] rounded-full animate-pulse"></div>
                <span className="text-gray-300 text-base md:text-lg font-bold">{title}: </span>
            </div>
            {children}
        </div >
    );
...
export default function PackageData() {
    ...
    return (
        <div className="flex justify-center min-h-screen py-14 md:py-20">
            <motion.div
                ...
                <div className="flex flex-col items-start justify-center mt-4 gap-1.5">
                    <PackageDetails title={"Name"}>
                        <span className="text-sm md:text-base font-medium text-gray-200/50" >{packageData?.packageVersion.package.name} </span>
                    </PackageDetails>

                    <PackageDetails title={"Ecosystem"}>
                        <span className="text-sm md:text-base font-medium text-gray-200/50">{packageData?.packageVersion.package.ecosystem}</span>
                    </PackageDetails>

                    <PackageDetails title={"Version"}>
                        <span className="text-sm md:text-base font-medium text-gray-200/50">
                    ...
                    <PackageDetails title={"Registries"}>
                        {packageData?.insight?.registries?.length > 0 ? (
                            ...
                            <span className="text-sm md:text-base font-medium text-gray-200/50">Not available</span>
                        )}
                    </PackageDetails>
                    ...
                    <div className="flex flex-col items-start justify-center gap-2 md:gap-1">
                        <div className="flex items-center justify-center gap-1">
                            <div className="size-1 md:size-2 bg-[#069c85] rounded-full animate-pulse"></div>
                        ...
                        {packageData?.insight?.projectInsights?.length > 0 ?
                            packageData?.insight.projectInsights.map((insight, i) => (
                                <div key={i} className="flex flex-col items-start justify-center gap-1 ml-7">
                                    ...
                                    <span className="text-sm md:text-base font-medium text-gray-200/50"> Stars : {insight?.stars ?? "Not mentioned"}</span>
                                    <span className="text-sm md:text-base font-medium text-gray-200/50"> Open Issues: {insight?.issues?.open ?? "Not mentioned"}</span>
